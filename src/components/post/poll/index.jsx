        ...
        <div className="mt-3 grid gap-1">
            {!poll.voted && poll.answers.map((answer) => (
                <button
                    ...
                    className="h-8 border border-[color:var(--color-primary)] rounded-full text-[color:var(--color-primary)] font-bold hover:bg-[var(--color-primary-alpha)] transition-colors"
                    key={answer.id}>
                    {answer.text}
                </button>
            ))}
            {poll.voted && poll.answers.map((answer) => (
                <div className="h-8 flex items-center justify-between relative overflow-hidden rounded-md px-2 z-[1]">
                    ...
                    >
                        {answer.text}
                        {answer?.voted && (
                            <svg viewBox="0 0 24 24" className="h-[1.25rem] ml-1">
                                    <path
                                        fill="currentColor"
